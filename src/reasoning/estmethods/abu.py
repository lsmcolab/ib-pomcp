...
class ABU(object):
    ...
    def __init__(self, env, template_types, parameters_minmax, grid_size=100, reward_factor=0.04, degree=2):
        ...
        self.estimate_adversary = env.is_adversarial()
            ...
            if tindex != adhoc_agent.index and tindex not in self.teammate:
                ...
                for type in self.template_types:
                    ...
                    if env.type_knowledge is False:
                        ...
                        self.teammate[teammate.index][type]['probability_history'] = [1.0] if type == teammate.type else [0.0]
                    ...
                    if env.parameter_knowledge is False:
                        ...
                        self.teammate[teammate.index][type]['parameter_estimation_history'] = [teammate.get_parameters()]
    ...
    def update(self,env):
        self.check_teammates_estimation_set(env)
        # ABU estimation requires, at least, one previous state in the history to start the estimation
        if self.previous_state is None:
            self.previous_state = env.copy()
            return self
        
        #####
        # START OF ABU ESTIMATION
        #####
        # Performing the ABU estimation
        ...
        for agent in env.components['agents']:
            ...
            if agent.index != adhoc_agent.index and self.is_in_the_previous_state(agent):
                ...
                new_estimation = self.parameter_estimation_update(env, y)
                ...
                last_types_probabilities = np.array([self.teammate[agent.index][t]['probability_history'][-1]\
    ...
    def parameter_estimation_update(self, env, y):
        # if we don't know the true parameter beforehand, estimate it
        if env.parameter_knowledge is False:
            new_estimation = self.abu_update(y)
            return new_estimation
        # else, just pick the correct one
        else:
            adhoc = env.get_adhoc_agent()
            new_estimation = []
            for ag in env.components['agents']:
                if ag.index != adhoc.index:
                    new_estimation.append(ag.get_parameters()) 
            return new_estimation
